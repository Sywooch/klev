...
class Cats extends \yii\db\ActiveRecord
{
    ...
    function view_cat($arr, $parent_id = 0)
    {
        ...
        for ($i = 0; $i < count($arr[$parent_id]); $i++) {
            ...
            $this->tree .= '<li>
                '.$var1.'
                <a class="pll" title="Перейти в категории ветки «'.$arr[$parent_id][$i]['name'].'»" href="' . Url::to(['/admin/cats/index?parent_id=' . $arr[$parent_id][$i]['id']]) . '">'
                ...
                '&nbsp;<span class="badge" title="Сортировка">'.$arr[$parent_id][$i]['sort'].'</span>';
    ...
    function view_cat_for_product($arr, $parent_id = 0,$config = [])
    {
        ...
        for ($i = 0; $i < count($arr[$parent_id]); $i++) {
            ...
            $this->tree .= '<li>'.$var1.$arr[$parent_id][$i]['name'].
             ((empty($arr[$arr[$parent_id][$i]['id']])) ? '<input type="radio" '.(($checked) ? ' checked ' : '').$checked.'class="adminradio"  value = "'.$arr[$parent_id][$i]['id'].'" name="cats[]" />' : '');
            //рекурсия - проверяем нет ли дочерних категорий
    ...
    public static function rootCats(){
        $cats = Cats::find()->where('active = 1 and parent_id = 166')->orderBy('sort ASC,id ASC')->all();
        $tmp = '';
        if ($cats){
            foreach ($cats as $key => $value) {
                $tmp.='<a href="'.Cats::get_url($value->id).'">'.$value->name.'</a>';
            }
        }
        return $tmp;
    }
    ...
    public static function get_url($cat_id)
    {
